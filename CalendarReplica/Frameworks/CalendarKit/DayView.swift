...
public protocol DayViewDelegate: AnyObject {
  ...
  func dayViewDidLongPressTimelineAtHour(_ hour: Int)
    func dayViewDidReleaseLongPressTimelineAtHour(_ hour: Int)
    func dayViewDidMoveLongPressTimelineAtHour(_ hour: Int)
...
extension DayView: TimelinePagerViewDelegate {
  ...
  public func timelinePagerDidReleaseLongPressTimelineAtHour(_ hour: Int) {
    delegate?.dayViewDidReleaseLongPressTimelineAtHour(hour)
  }
    public func timelinePagerDidMoveLongPressTimelineAtHour(_ hour: Int) {
        delegate?.dayViewDidReleaseLongPressTimelineAtHour(hour)
    }
...
extension DayView: TimelineViewDelegate {
  public func timelineView(_ timelineView: TimelineView, didLongPressAt hour: Int) {
    ...
    public func timelineView(_ timelineView: TimelineView, didReleaseLongPressAt hour: Int) {
        delegate?.dayViewDidReleaseLongPressTimelineAtHour(hour)
    }
    public func timelineView(_ timelineView: TimelineView, didMoveLongPressAt hour: Int) {
        delegate?.dayViewDidMoveLongPressTimelineAtHour(hour)
    }
}
